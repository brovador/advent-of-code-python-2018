...
class Grid():
	...
	def hash(self):
		return (self.resource_value(), ''.join([''.join(x) for x in self.data]))
...
def main(test = ''):
	...
	loop_start = hashes.index(grid.hash())
	loop_length = len(hashes) - loop_start
	
	result_idx = (minutes - loop_start) % loop_length
	result_idx = loop_start + result_idx
	print hashes[result_idx][0]
